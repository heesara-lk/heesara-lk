...
const PORONDAM_20=[{id:1,name_si:'නැකැත්'},{id:2,name_si:'ගණ'},{id:3,name_si:'යෝනි'},{id:4,name_si:'රාශි'},{id:5,name_si:'රාශි අධිපති'},{id:6,name_si:'වෛශ්ය'},{id:7,name_si:'දින'},{id:8,name_si:'මහේන්ද්ර'},{id:9,name_si:'ස්ත්රී දීර්ඝ'},{id:10,name_si:'යෝනි'},{id:11,name_si:'රජ්ජු'},{id:12,name_si:'වේධ'},{id:13,name_si:'වර්ණ'},{id:14,name_si:'නදී'},{id:15,name_si:'ග්රහ මෛත්රී'},{id:16,name_si:'භූත'},{id:17,name_si:'ගෝත්ර'},{id:18,name_si:'ලිංග'},{id:19,name_si:'පක්ෂි'},{id:20,name_si:'ආයු'},];
...
function calculateMatchingBreakdown(viewer:any, candidate:any){
  ...
  return {
    ...
    total:Math.round(porData.percent*0.40+ageScore*0.20+districtScore*0.10+religionScore*0.10+casteScore*0.10+jobScore*0.05+heightScore*0.05)
  };
}

export default function MatchesPage(){
  const router=useRouter(); const [matches,setMatches]=useState<any[]>([]); const [currentProfile,setCurrentProfile]=useState<any>(null); const [myIds,setMyIds]=useState<string[]>([]); const [myProfiles,setMyProfiles]=useState<any[]>([]); const [loading,setLoading]=useState(true); const [interests,setInterests]=useState<any>({}); const [isAdmin,setIsAdmin]=useState(false); const [myEmail,setMyEmail]=useState('');
  async function load(curIdOverride?:string){
    ...
    setMatches(scored); setLoading(false);
  }
  useEffect(()=>{ load(); },[]);
  ...
  return (
    <div className='max-w-5xl mx-auto p-4 bg-[#FFFBEB] min-h-screen'>
      ...
      {myProfiles.length>0 && (<div className='bg-white border-2 border-blue-200 p-3 rounded-xl mb-4'><div className='font-bold mb-2'>Switch Active Profile: ({myProfiles.length}/{isAdmin?'∞':2}) {isAdmin?`👑 Admin ${myEmail} ` :myEmail}</div><div className='flex gap-2 flex-wrap'>{myProfiles.map((p:any)=>(<button key={p.id} onClick={()=>load(p.id)} className={`px-4 py-2 rounded-full border font-bold ${currentProfile?.id===p.id?'bg-blue-600 text-white':'bg-white'}`}>{p.name||p.full_name} - {p.age}y {currentProfile?.id===p.id?' (Active)':''} {p.subscription_status} {p.is_visible===false?' (Hidden)':''} {p.religion? `(${p.religion})`:''}</button>))}</div></div>)}
      {currentProfile && <div className='bg-blue-50 border p-3 rounded-xl mb-4'><div className='font-bold'>Viewing as: {currentProfile.name||currentProfile.full_name} - {currentProfile.age}y - {currentProfile.religion||'Any'} - Living: {currentProfile.living_city||currentProfile.current_city} {currentProfile.is_private? '🔒 Private':''} | Status: {currentProfile.subscription_status}</div><div className='text-xs mt-1'>Expecting: Age {currentProfile.expectation_age_min}-{currentProfile.expectation_age_max} | Height {currentProfile.expectation_height_min}-{currentProfile.expectation_height_max} | Religion {currentProfile.expectation_religion||'Any'} | District {currentProfile.expectation_district||'Any'} | Caste {currentProfile.expectation_caste||'Any'} | Job {currentProfile.expectation_job||'Any'}</div></div>}
      <h1 className='text-xl font-bold mb-4'>Top Matches ({matches.length}) - All Mutual 100/70/40/10</h1>
      <div className='grid gap-4'>{matches.map((p:any)=>{
